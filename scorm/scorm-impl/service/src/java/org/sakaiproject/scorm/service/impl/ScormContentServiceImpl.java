...
public abstract class ScormContentServiceImpl implements ScormContentService, ScormConstants {
	...
	private ContentPackage convertToContentPackage(String resourceId, IValidator validator, IValidatorOutcome outcome) throws Exception {
		...
		cp.setReleaseOn(c.getTime());
		//Due and Accept dates are 1 week from now and 5pm by default
		c.add(Calendar.DAY_OF_YEAR, 7);
		c.set(Calendar.HOUR_OF_DAY, 17);
		c.set(Calendar.MINUTE, 0);
		c.set(Calendar.SECOND, 0);
		c.set(Calendar.MILLISECOND, 0);
		cp.setDueOn(c.getTime());
		cp.setAcceptUntil(c.getTime());
	...
	public void removeContentPackage(long contentPackageId) throws ResourceNotDeletedException {
		...
		if (lms.canDelete(lms.currentContext())) {
			...
			//check if GB items exist:
			GradebookExternalAssessmentService service = gradebookExternalAssessmentService();
			String context = lms.currentContext();
			boolean isGradebookDefined = service.isGradebookDefined(context);
			if(isGradebookDefined){
				...
				for (LaunchData launchData : launchDatas) {
					...
					if ("sco".equalsIgnoreCase(scormType)) {
						String assessmentExternalId = getAssessmentExternalId(contentPackageId, launchData.getItemIdentifier());
						boolean has = service.isExternalAssignmentDefined(lms.currentContext(), assessmentExternalId);
						if(has){
							//need to remove it:
							service.removeExternalAssessment(context, assessmentExternalId);
						}
					}
				}
			}
	...
	protected String getAssessmentExternalId(final long contentPackageId, String dataItemIdentifier) {
		String assessmentExternalId = "" + contentPackageId + ":" + dataItemIdentifier;
