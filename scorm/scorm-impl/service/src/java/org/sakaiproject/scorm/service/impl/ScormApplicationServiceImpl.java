...
public abstract class ScormApplicationServiceImpl implements ScormApplicationService, ScormConstants {
	...
	public void discardScoBean(String scoId, SessionBean sessionBean, INavigable agent) {
		...
		if (sessionBean.isCloseOnNextTerminate()) {
			if (log.isDebugEnabled()) {	log.debug("NYU-SCORM-DEBUG -- discardScoBean -- SessionBean: " + sessionBean + "; Agent: " + agent + "; scoId: " + scoId + "; RequestValue: " + request);	}
	...
	public boolean setValue(String dataModelElement, String value, SessionBean sessionBean, ScoBean scoBean) {
		...
		// already terminated
		if (scoBean != null && scoBean.isTerminated()) {
	...
	protected void synchResultWithGradebook(SessionBean sessionBean) {
		...
		if ("normal".equals(mode) && "completed".equals(completionStatus) && "credit".equals(credit)) {
			...
			if(ContentPackage.STORE_HIGHEST_GRADE.equals(sessionBean.getContentPackage().getStoreGradeOption())){
				//make sure you only update the GB with the highest score from all attempts
				List<IDataManager> allDataManagers = dataManagerDao().find(contentPackageId, sessionBean.getLearnerId());
				for(IDataManager dbDataManager : allDataManagers){
					mode = getValueAsString("cmi.mode", dbDataManager); // passed, failed, unknown
					credit = getValueAsString("cmi.credit", dbDataManager); // credit, no_credit 
					completionStatus = getValueAsString("cmi.completion_status", dbDataManager); // (completed, incomplete, not_attempted, unknown)
					if ("normal".equals(mode) && "completed".equals(completionStatus) && "credit".equals(credit)) {
						if(getRealValue("cmi.score.scaled", dbDataManager) * 100d > score){
							score = getRealValue("cmi.score.scaled", dbDataManager) * 100d;
						}
					}
				}
			}
	...
	public void syncAllGradesWithGradebook(String context, String assessmentExternalId, ContentPackage contentPackage){
		for(Learner learner : learnerDao().find(context)){
			String learnerId = learner.getId();
			List<IDataManager> allDataManagers = dataManagerDao().find(contentPackage.getContentPackageId(), learnerId);
			Double score = null;
			Long lastTimeStamp = null;
			for(IDataManager dbDataManager : allDataManagers){
				String mode = getValueAsString("cmi.mode", dbDataManager); // passed, failed, unknown
				String credit = getValueAsString("cmi.credit", dbDataManager); // credit, no_credit 
				String completionStatus = getValueAsString("cmi.completion_status", dbDataManager); // (completed, incomplete, not_attempted, unknown)
				String timeStampStr = getValueAsString("cmi.timestamp", dbDataManager);
				Long timeStamp = null;
				...
				if ("normal".equals(mode) && "completed".equals(completionStatus) && "credit".equals(credit)) {
					if(ContentPackage.STORE_MOST_RECENT_GRADE.equals(contentPackage.getStoreGradeOption())){
						if(lastTimeStamp == null || (timeStamp != null && timeStamp > lastTimeStamp)){
							score = getRealValue("cmi.score.scaled", dbDataManager) * 100d;
						}
					}else if(ContentPackage.STORE_HIGHEST_GRADE.equals(contentPackage.getStoreGradeOption()) 
							&& (score == null || getRealValue("cmi.score.scaled", dbDataManager) * 100d > score)){
						score = getRealValue("cmi.score.scaled", dbDataManager) * 100d;
					}
				}
			}
			if(score != null){
				//we have a score to record... add it
